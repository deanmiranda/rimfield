...
# Legacy inventory scene removed - using pause menu for inventory now
...
# Legacy inventory instantiation removed - using pause menu for inventory now
...
# Legacy inventory toggle removed - using pause menu for inventory now
...
func update_input_processing() -> void:
	...
	if current_scene and current_scene.name != "Main_Menu": # Enable input only in game scenes
		...
		# Hide pause menu when switching to non-game scenes
		if pause_menu:
			pause_menu.visible = false
